...
class Application extends BaseApp {
  ...
  init(frameworkConfig, frameworkOptions, appConfig, envConfig) {
    ...
    */
    ...
    this.wait('LOAD_PLUGIN');
    let loadPlugin = () => {
      // merge config again, for DECRYPT_CONFIG
      Object.assign(appConfig, this.options);
      /**
       * 挂载配置的middleware
       */
      ...
      this.ready('LOAD_PLUGIN');
    }
    process.nextTick(() => {
      if (this.checkReady('DECRYPT_CONFIG') !== undefined) {
        if (this.checkReady('DECRYPT_CONFIG')) {
          loadPlugin();
        } else {
          this.on('DECRYPT_CONFIG', loadPlugin);
        }
      } else {
        if (this.checkReady('BUILDIN_CONFIG')) {
          loadPlugin()
        } else {
          this.on('BUILDIN_CONFIG', loadPlugin);
        }
      }
    })
  }
  ...
  run(cb, config) {
    ...
    this.ready(() => {
       ...
       */
      if (self.config.fallbackHandler) {
        self.express.use(self.config.fallbackHandler);
      }

      let customErrorHandler = self.config.errorHandler;
      self.express.use(function (err, req, res, next) {   // eslint-disable-line
        ...
        self.log.error('REQUEST_ERROR method:', req.method, 'url:', req.url, 'rid:', rid, 'code:', code, 'message:', err.stack || err);
        customErrorHandler(err, req, res, (err) => {
          ...
          if (type === 'json') {
            if (wrapJson) {
              err = wrapJson({code, message}, null, rid);
            }
            res.json(err);
          } else if (type === 'jsonp') {
            if (wrapJson) {
      ...
      if (config) {
        serv.listen(config.port, config.ip || '', function (err) {
        ...
        let sock;
        if (self.options.targetSock || self.options.targetPort) {
          sock = self.options.targetSock || self.options.targetPort;
        } else {
        ...
        serv.listen(sock, function () {
          ...
          cb && cb(null, {
            ...
            static: self.options.honeycomb.static,
            target: sock,
  ...
  */
  ...
  mountMiddleWare() {
    ...
    if (this.options.orderedMiddleware) {
      ...
      this.log.warn('❗️❗️carefully❗️ you are using ordered middleware❗️❗️');
      this.log.warn('mount middleware by custom order', middlewareList.join(',')); 
    } else {
    ...
    mds = mds.map((middleware) => {
      ...
      if (middleware.constructor.name !== 'GeneratorFunction') {
        ...
        try {
          ...
          if (!middleware || !middleware.constructor) {
            throw new Error('middleware illegal');
          }
        } catch (e) {
          e.message = `init middleware "${id}" error, ` + e.message;
          throw e;
        }
      }
      /**
       * 注意不要调换这个if逻辑的顺序，也不要合并这个if逻辑到上一个if
       * 因为前面的if逻辑中，可能得到一个generatorFunction的middleware
       */
      if (middleware.constructor.name === 'GeneratorFunction') {
        middleware = utils.wrapMiddleware(middleware);
      }
  ...
  wrapRouter(router) {
    ...
    ['get', 'post', 'delete', 'put', 'patch', 'options', 'trace', 'head', 'all'].forEach(function (method) {
