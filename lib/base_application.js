...
class BaseApplication extends Events {
  ...
  constructor() {
    ...
    options.targetPort = envConfig.targetPort;
    // 加载应用的环境配置
  ...
  init(frameworkConfig, frameworkOptions, appConfig, envConfig) {
    ...
    this.emit('BUILDIN_CONFIG');
    this.ready('BUILDIN_CONFIG');
  }
  onConfigReady(fn) {
    this.wait('DECRYPT_CONFIG');
    let done = (err) => {
      if (err) {
        log.error('server onConfigReadyHook error', err)
        process.exit(1)
      }
      this.emit('DECRYPT_CONFIG');
      this.ready('DECRYPT_CONFIG');
    };
    if (this.checkReady('BUILDIN_CONFIG')) {
      fn(done);
    } else {
      this.on('BUILDIN_CONFIG', () => {
        fn(done)
      });
    }
  }
  ...
  handleException() {
    ...
    process.on('unhandledRejection', (reason, p) => {
      this.log.error(`Unhandled Rejection at: Promise ${p}, reason: ${reason && reason.stack}`);
    });
